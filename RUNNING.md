...
Open **https://yourIP:8443** (accept the one-time self-signed cert warning).

### Fixed IP + custom ports (`make start` / `make stop`)

To run on **yourIP** with a custom port range (2002, 2004–2010 — 2003 is
skipped as reserved), use the convenience targets backed by
`scripts/run_local.sh`:

```bash
make start     # → https://yourIP:2002   (Caddy TLS, the browser URL)
...
AURALYNQ_API_FALLBACK=https://yourIP:8443/api
